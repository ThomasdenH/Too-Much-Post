...
use ggez::*;
use ggez::graphics::{Point2, DrawParam, Vector2, Text, Drawable, Rect, FilterMode};
...
const SPAWN_TIME: f32 = 10.0;

const HOUSE_COUNT: u32 = 5;
...
impl House {
    fn new(ctx: &mut Context, position: Point2, number: u32, image_count: usize) ->
                                                                               GameResult<House> {
    ...
    fn draw(&self, ctx: &mut Context, game: &Game) -> GameResult<()> {
        ...
        let number_dest = Game::float_coord_to_screen(ctx,
                                                      self.position - game.player.coords +
                                                          Vector2::new(1.0, 1.3));
        self.text.draw_ex(ctx, DrawParam {
            dest: number_dest,
            rotation: 0.0,
            offset: Point2::new(0.5, 0.5),
            shear: Point2::new(0.0, 0.0),
...
impl Letter {
    ...
    fn draw(&self, ctx: &mut Context, game: &Game) -> GameResult<()> {
        ...
        game.images.letter.draw_ex(ctx, DrawParam {
            dest,
            rotation: 0.0,
            offset: Point2::new(0.5, 0.5),
            shear: Point2::new(0.0, 0.0),
    ...
    fn draw_in_hand(&self, ctx: &mut Context, game: &Game, i: usize) -> GameResult<()> {
        let scale = Game::tile_size(ctx) / 32.0;
        let dest = Game::float_coord_to_screen(ctx, Point2::new(
            scale[0] * 0.5,
            scale[0] * 0.5 + scale[1] * 0.6 * i as f32
        ));
...
impl<'a> Game<'a> {
    pub fn new(ctx: &mut Context, images: &'a Images) -> GameResult<Game<'a>> {
        ...
        for house_number in 0..HOUSE_COUNT {
            loop {
                ...
                let new_house = House::new(ctx, position, 1 + house_number, images.houses.len())?;
                if houses.iter().all(|house: &House| !house.intersects(&new_house)) {
    ...
    fn draw_trees(&mut self, ctx: &mut Context) -> GameResult<()> {
        let scale = Game::tile_size(ctx) / 32.0;
        for i in 0..LEVEL_HEIGHT - 8 {
            ...
            self.images.trees.draw_ex(ctx, DrawParam {
                dest,
                rotation: 0.0,
                offset: Point2::new(0.5, 0.5),
                shear: Point2::new(0.0, 0.0),
...
impl<'a> EventHandler for Game<'a> {
    fn update(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        if self.game_over {
            ...
            while self.time_since_last_letter > SPAWN_TIME {
                self.time_since_last_letter -= SPAWN_TIME;
                let position = Point2::new(
                    thread_rng().gen_range(0.0, LEVEL_WIDTH as f32),
                    thread_rng().gen_range(0.0, LEVEL_HEIGHT as f32),
                );
            ...
            while self.letters.len() > 0 {
                ...
                if letter.player_intersection(ctx, &self) {
                    ...
                    if self.holding_letters.len() > MAX_HOLDING {
                        ...
                        dropped_letter.position = Game::screen_coord_to_float(ctx,
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        for animated_letter in &self.letter_animation {
            self.images.letter.draw_ex(
                ctx,
                DrawParam {
                    dest: player_pos + Vector2::new(0.0, -20.0 + animated_letter * -32.0),
                    rotation: 0.0,
                    offset: Point2::new(0.5, 0.5),
                    ..Default::default()
                }
        ...
        self.images.arrow.draw_ex(ctx, DrawParam {
        ...
        self.score_text.draw(ctx, Point2::new(w - 100.0, 10.0), 0.0)?;

        // Draw progress bar
        self.images.progress_bar.draw_ex(ctx, DrawParam {
            dest: Point2::new(
                w - 100.0,
                scale[0] * 1.0 * 32.0
            ),
        ...
        self.images.progress_bar_filled.draw_ex(ctx, DrawParam {
            src: Rect {
                x: 0.0,
                y: 0.0,
    ...
    fn key_down_event(&mut self, _ctx: &mut Context, keycode: Keycode, _keymod: Mod, _repeat:
    bool) {
        match keycode {
